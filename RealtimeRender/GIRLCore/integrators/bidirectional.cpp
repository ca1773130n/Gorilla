...
//	Date    :	Modifications
//	=========================== 
//	2008-10-04  :   First version - bidirectional.cpp initial implementation
//
	...
	vector<BidirVertex> &vertices, float directWeight, float time)
{
	...
	while (nVerts < vertices.size()) {
		...
		v.fgirl = v.f;
		v.fgirl *= v.cosi / v.pdfR;
		v.rrR = min<float>(1.f, v.fgirl.filter(tspack));
		if (nVerts > 3) {
			if (v.rrR < data[0])
				break;
			v.fgirl /= v.rrR;
		...
		if (nVerts > 1)
			v.fgirl *= vertices[nVerts - 2].fgirl;
	...
	vector<BidirVertex> &vertices, float time)
{
	...
	while (nVerts < vertices.size()) {
		...
		v.f *= AbsDot(v.wi, v.ns) / v.cosi;
		v.fgirl = v.f;
		v.fgirl *= v.coso / v.pdf;
		v.rr = min<float>(1.f, v.fgirl.filter(tspack));
		if (nVerts > 3) {
			if (v.rr < data[0])
				break;
			v.fgirl /= v.rr;
	...
	float pdfLightDirect, bool isLightDirect, float *weight)
{
	...
	*weight = 0.f;
	...
	if (nLight > 0 && nEye > 0) {
		...
		eyeV.fgirl = eyeV.f; // No pdf as it is a direct connection
		if (nEye > 1)
			eyeV.fgirl *= eye[nEye - 2].fgirl;
		// Prepare light vertex for connection
		lightV.coso = AbsDot(lightV.wo, lightV.ng);
		lightV.d2 = eyeV.d2;
		lightV.f *= AbsDot(lightV.wi, lightV.ns) / lightV.cosi;
		lightV.fgirl = lightV.f; // No pdf as it is a direct connection
		if (nLight > 1)
			lightV.fgirl *= light[nLight - 2].fgirl;
		// Connect eye and light vertices
		L = eyeV.fgirl * G(eyeV, lightV) * lightV.fgirl;
		if (L.Black())
			return 0.f;
		L *= fConnect;
		...
		L = light[nLight - 2].fgirl;//FIXME: not implemented yet
		// Evaluate factors for path weighting
		v.dARWeight = v.ePdf;
		v.flags = BxDFType(~BSDF_SPECULAR);
