...
//	Date    :	Modifications
//	=========================== 
//	2008-10-04  :   First version - fleximage.cpp initial implementation
//
...
void FlexImageFilm::CreateBuffers()
{
    ...
    if(writeResumeFlm && !restartResumeFlm) {
        ...
        if(ifs.good()) {
            // Dade - read the data
            girlError(GIRL_NOERROR, GIRL_INFO, (std::string("Reading film status from file ")+fname).c_str());
...
void FlexImageFilm::WriteResumeFilm(const string &filename)
{
	...
	girlError(GIRL_NOERROR, GIRL_INFO, (std::string("Writing film status to file ") +
			filename).c_str());
...
{
	std::stringstream ss;
	ss << "Writing Tonemapped TGA image to file " << filename << "\n";
//	girlProgressMsgPrint(GIRL_NOERROR,GIRL_INFO,ss.str().c_str());

	//moonhpark
	char buf[256] = {""};
	strcpy (buf, (std::string("Writing Tonemapped TGA image to file: ")+filename).c_str());
...
void FlexImageFilm::WriteEXRImage(vector<Color> &rgb, vector<float> &alpha, const string &filename)
{
	...
	std::stringstream ss;
	ss << "Writing OpenEXR image to file " << filename << "\n";
//	girlProgressMsgPrint(GIRL_NOERROR,GIRL_INFO,ss.str().c_str());
...
void FlexImageFilm::WriteIGIImage(vector<Color> &rgb, vector<float> &alpha, const string &filename)
{
	// Write IGI image
	std::stringstream ss;
	ss << "Writing IGI image to file " << filename << "\n";
//	girlProgressMsgPrint(GIRL_NOERROR,GIRL_INFO,ss.str().c_str());
...
//        boost::recursive_mutex::scoped_lock lock(addSampleMutex);
	...
	if(!os.good()) {
		girlError(GIRL_SYSTEM, GIRL_SEVERE, "Error preparing data for film resume file");
		delete pixelBuf;
		return;
	}
	...
	if(!stream.good()) {
		girlError(GIRL_SYSTEM, GIRL_SEVERE, "Error writing film resume file");
		delete pixelBuf;
		return;
	}
...
//            boost::recursive_mutex::scoped_lock lock(addSampleMutex);
            ...
            girlError(GIRL_SYSTEM, GIRL_ERROR, "Error while reading samples");
        }
    } else {
        girlError(GIRL_SYSTEM, GIRL_ERROR, "Error while reading samples");
...
Film* FlexImageFilm::CreateFilm(const ParamSet &params, Filter *filter)
{
	...
	// output filenames
	string filename = params.FindOneString("filename", "girlout");
