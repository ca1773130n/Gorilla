...
ri_stmt: ACCELERATOR STRING paramlist
{
	...
	Context::girlAccelerator($2, params);
	FreeArgs();
}
| AREALIGHTSOURCE STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlAreaLightSource($2, params);
	FreeArgs();
}
| ATTRIBUTEBEGIN
{
...
| CAMERA STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlCamera($2, params);
	FreeArgs();
}
| CONCATTRANSFORM num_array
{
	if (VerifyArrayLength( $2, 16, "ConcatTransform" ))
		Context::girlConcatTransform( (float *) $2->array );
	ArrayFree( $2 );
}
| COORDINATESYSTEM STRING
{
	Context::girlCoordinateSystem( $2 );
}
| COORDSYSTRANSFORM STRING
{
	Context::girlCoordSysTransform( $2 );
}
| FILM STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlFilm($2, params);
	FreeArgs();
}
| IDENTITY
{
...
| LIGHTSOURCE STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlLightSource($2, params);
	FreeArgs();
}
| LOOKAT NUM NUM NUM NUM NUM NUM NUM NUM NUM
{
	Context::girlLookAt($2, $3, $4, $5, $6, $7, $8, $9, $10);
}
| MATERIAL STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlMaterial($2, params);
	FreeArgs();
}
| MAKENAMEDMATERIAL STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlMakeNamedMaterial($2, params);
	FreeArgs();
}
| NAMEDMATERIAL STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlNamedMaterial($2, params);
	FreeArgs();
}
| OBJECTBEGIN STRING
{
	Context::girlObjectBegin($2);
}
| OBJECTEND
{
...
{
	Context::girlObjectInstance($2);
}
| PIXELFILTER STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlPixelFilter($2, params);
	FreeArgs();
}
| REVERSEORIENTATION
{
...
{
	Context::girlRotate($2, $3, $4, $5);
}
| SAMPLER STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlSampler($2, params);
	FreeArgs();
}
| SCALE NUM NUM NUM
{
	Context::girlScale($2, $3, $4);
}
| SEARCHPATH STRING
{
	;//girlSearchPath($2);
...
| PORTALSHAPE STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlPortalShape($2, params);
	FreeArgs();
}
| SURFACEINTEGRATOR STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlSurfaceIntegrator($2, params);
	FreeArgs();
}
| TEXTURE STRING STRING STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlTexture($2, $3, $4, params);
	FreeArgs();
}
| TRANSFORMBEGIN
{
...
| TRANSFORM real_num_array
{
	if (VerifyArrayLength( $2, 16, "Transform" ))
		Context::girlTransform( (float *) $2->array );
	ArrayFree( $2 );
}
...
| VOLUMEINTEGRATOR STRING paramlist
{
	ParamSet params;
	InitParamSet(params, CPS, CPT, CPA, CPSZ, CPTH);
	Context::girlVolumeIntegrator($2, params);
	FreeArgs();
}
