	...
	u_int nCausticPhotons, LightPhotonMap *causticMap) 
{
	...
	if (mapFileName) {
        ...
        if(ifs.good()) {
			...
			girlError(GIRL_NOERROR, GIRL_INFO, "Reading radiance photon map...");
			RadiancePhotonMap::load(ifs, radianceMap);
			ss.str("");
			ss << "Read " << radianceMap->getPhotonCount() << " radiance photons";
			girlError(GIRL_NOERROR, GIRL_INFO, ss.str().c_str());

			girlError(GIRL_NOERROR, GIRL_INFO, "Reading indirect photon map...");
			LightPhotonMap::load(ifs, indirectMap);
			ss.str("");
			ss << "Read " << indirectMap->getPhotonCount() << " light photons";
			girlError(GIRL_NOERROR, GIRL_INFO, ss.str().c_str());

			girlError(GIRL_NOERROR, GIRL_INFO, "Reading caustic photon map...");
			...
			girlError(GIRL_NOERROR, GIRL_INFO, "Photon maps file doesn't exist yet");
			ifs.close();
		}
	}
    ...
    while (!radianceDone || !directDone || !causticDone || !indirectDone) {
		...
		if (nshot > 500000) {
			if (indirectDone && unsuccessful(nCausticPhotons, causticPhotons.size(), nshot)) {
				...
				// enough photons
				girlError(GIRL_CONSISTENCY, GIRL_WARNING, "Unable to store enough photons in the caustic photonmap. Giving up and disabling the map.");
			...
			if(unsuccessful(nIndirectPhotons, indirectPhotons.size(), nshot)) {
				girlError(GIRL_CONSISTENCY, GIRL_ERROR, "Unable to store enough photons in the indirect photonmap. Unable to render the image.");
				return;
			}
        }
    ...
    if (finalGather) {
		girlError(GIRL_NOERROR, GIRL_INFO, "Computing radiance photon map...");
	...
	if (mapFileName && !mapsFileExist) {
		girlError(GIRL_NOERROR, GIRL_INFO, "Saving photon maps to file" );
