...
//	Date    :	Modifications
//	=========================== 
//	2008-10-04  :   First version - color.h initial implementation
//
...
class GIRLCOREDLL   RGBColor : public Color {
public:
	// RGBColor Public Methods
	RGBColor(float v = 0.f) {
			c[0] = v; c[1] = v; c[2] = v; alpha = 0;
	}
	RGBColor(const float cs[3]) {
			c[0] = cs[0]; c[1] = cs[1]; c[2] = cs[2]; alpha = 0;
	}
	RGBColor(XYZColor xyz);
	RGBColor(const Color &color) { // so that operators work
		c[0] = color.c[0]; c[1] = color.c[1]; c[2] = color.c[2]; alpha = 0;
	}
...
class GIRLCOREDLL   XYZColor : public Color {
    // Dade - serialization here is required by network rendering
    friend class boost::serialization::access;
