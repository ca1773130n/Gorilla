	...
	const ParamSet &paramSet)
{
	if (DynamicLoader::registeredShapes().find(name) !=
		...
		int numShapes = DynamicLoader::registeredShapesPointer().size();
		char szNum[255];
		itoa( numShapes, szNum, 10 );
		std::pair<string, boost::shared_ptr<Shape> > newPair(name + string(szNum), ret);				
		DynamicLoader::registeredShapesPointer().insert(newPair);
	...
	const Transform &mtl2world, const TextureParams &mp, const string& matname)
{
	if (DynamicLoader::registeredMaterials().find(name) !=
		...
		ret->SetType(name);
		ret->SetName(matname);
		std::pair<string, boost::shared_ptr<Material> > newPair(matname, ret);				
		DynamicLoader::registeredMaterialsPointer().insert(newPair);

		std::stringstream ss;
		ss << "Making material " << name.c_str();
		girlError(GIRL_WARNING, GIRL_WARNING, ss.str().c_str());
...
map<string, boost::shared_ptr<Shape> > &DynamicLoader::registeredShapesPointer()
{
	static map<string, boost::shared_ptr<Shape> > *Map = new map<string, boost::shared_ptr<Shape> >;
	return *Map;
}

map<string, boost::shared_ptr<Material> > &DynamicLoader::registeredMaterialsPointer()
{
	static map<string, boost::shared_ptr<Material> > *Map = new map<string, boost::shared_ptr<Material> >;
	return *Map;
}
