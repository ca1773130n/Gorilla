...
int Scene::FilmHaltspp() {
	return camera->film->haltSamplePerPixel;
}
...
double Scene::Statistics(const string &statName) {
	...
	else if(statName=="haltspp")
		return FilmHaltspp();
    else {
        std::string eString("girlStatistics - requested an invalid data : ");
        eString+=statName;
        girlError(GIRL_BADTOKEN, GIRL_ERROR, eString.c_str());
        return 0.;
    }
}
...
Scene::Scene(Camera *cam, SurfaceIntegrator *si,
    ...
    lastTime = 0.;
    if (lts.size() == 0) {
        girlError(GIRL_MISSINGDATA, GIRL_SEVERE, "No light sources defined in scene; nothing to render. Exitting...");
        exit(1);
    }
