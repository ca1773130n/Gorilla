...
//	Date    :	Modifications
//	=========================== 
//	2008-10-04  :   First version - MerlWard.cpp initial implementation
//
...
BSDF *MerlWard::GetBSDF(const TsPack *tspack, const DifferentialGeometry &dgGeom, const DifferentialGeometry &dgShading, float u) const {
	...
	if(!ks.Black())
		bsdf->Add(BSDF_ALLOC(tspack, WardBRDF)(ks, alpha, alpha, gamma));

	bsdf->Add(BSDF_ALLOC(tspack, Lambertian)(kd));

	return bsdf;
}
// MerlWard Dynamic Creation Routine
Material * MerlWard::CreateMaterial(const Transform &xform,
	...
	return new MerlWard(material_name, reflection_type, bumpMap);
}

static DynamicLoader::RegisterMaterial<MerlWard> r("MerlWard");