...
//	Date    :	Modifications
//	=========================== 
//	2008-10-04  :   First version - metal.cpp initial implementation
//
...
bool ReadGirlpopData(std::ifstream &fs, vector<float> &wl, vector<float> &n, vector<float> &k) {
...
void IORFromName(const string name, vector<float> &wl, vector<float> &n, vector<float> &k) {
	...
	if (name == "amorphous carbon") {
		...
		if (name != "aluminium") {
			...
			girlError(GIRL_NOERROR, GIRL_WARNING, msg.c_str());
		}
		ns = SopraSamples;
		sw = SopraWavelengths;
		sn = AluminiumN;
