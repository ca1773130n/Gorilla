...
void CMeshLoader10::Destroy()
{
	...
	{
		OBJMaterial *pMaterial = m_Materials.GetAt( iMaterial );
...
HRESULT CMeshLoader10::LoadGeometryFromOBJ( const WCHAR* strFileName )
{
	...
	OBJMaterial* pMaterial = new OBJMaterial();
	if( pMaterial == NULL )
	...
	for(; ; )
	{
		...
		else if( 0 == wcscmp( strCommand, L"usemtl" ) )
		{
			...
			//m_pGIRLNamedMaterials = &Context::girlGetNamedMaterial();

			bool bFound = false;
			for( int iMaterial = 0; iMaterial < m_Materials.GetSize(); iMaterial++ )
			{
				OBJMaterial* pCurMaterial = m_Materials.GetAt( iMaterial );
			...
			if( !bFound )
			{
				...
				// Check the material from the name
				char szTemp[256];
				WideCharToMultiByte( CP_ACP, 0, strName, -1, szTemp, 256, 0, 0 );
				string szName( szTemp );
				map<string, boost::shared_ptr<Material> >::iterator iter;

				map<string, boost::shared_ptr<Material> > matMap = DynamicLoader::registeredMaterialsPointer();
				iter = matMap.find(szName);
				Material* pMaterial = (Material*)iter->second.get();
				GIRLMaterial* pGIRLMat = (GIRLMaterial*)pMaterial;
				
				CGMaterial* pNewGMat = new CGMaterial( pGIRLMat );

				switch( pNewGMat->GetGIRLMaterial()->GetGIRLMaterialType() )
				{
				case GIRL_MATERIAL_BRDF_LAMBERT:
					pNewGMat->SetupLambert();
					break;
				...
				m_GMaterials.push_back( *pNewGMat );
...
HRESULT CMeshLoader10::LoadMaterialsFromMTL( const WCHAR* strFileName )
{
	...
	for(; ; )
	{
		...
		if( 0 == wcscmp( strCommand, L"newmtl" ) )
		{
			...
			{
				OBJMaterial* pCurMaterial = m_Materials.GetAt( i );
				if( 0 == wcscmp( pCurMaterial->strName, strName ) )
				{
...
//--------------------------------------------------------------------------------------
void CMeshLoader10::InitMaterial( OBJMaterial* pMaterial )
{
