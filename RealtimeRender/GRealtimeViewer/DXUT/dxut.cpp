...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3d9DeviceFromApp,
                          ID3D10Device* pd3d10DeviceFromApp, bool bForceRecreate, bool bClipWindowToSingleAdapter, bool bSwapHandedness = FALSE );
...
HRESULT DXUTSetupD3D10Views( ID3D10Device* pd3dDevice, DXUTDeviceSettings* pDeviceSettings, bool bSwapHandedness = FALSE );
HRESULT DXUTCreate3DEnvironment10( ID3D10Device* pd3dDeviceFromApp, bool bSwapHandedness = FALSE );
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateDevice( bool bWindowed /*= true*/, bool bSwapHandedness /*= FALSE*/, int nSuggestedWidth /*= 0*/, int nSuggestedHeight /*= 0 */ )
{
    HRESULT hr = S_OK;
    ...
    hr = DXUTChangeDevice( &deviceSettings, NULL, NULL, false, true, bSwapHandedness );
    if( FAILED( hr ) )
        return hr;
...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings,
                          IDirect3DDevice9* pd3d9DeviceFromApp, ID3D10Device* pd3d10DeviceFromApp,
                          bool bForceRecreate, bool bClipWindowToSingleAdapter, bool bSwapHandedness )
{
    HRESULT hr;
...
HRESULT DXUTSetupD3D10Views( ID3D10Device* pd3dDevice, DXUTDeviceSettings* pDeviceSettings, bool bSwapHandedness /*= FALSE */ )
{
    ...
    if( pDeviceSettings->d3d10.AutoCreateDepthStencil )
    {
		...
		RSDesc.FrontCounterClockwise = bSwapHandedness ? FALSE : TRUE;
        RSDesc.DepthBias = 0;
...
// Creates the 3D environment
//--------------------------------------------------------------------------------------
HRESULT DXUTCreate3DEnvironment10( ID3D10Device* pd3d10DeviceFromApp, bool bSwapHandedness /*= FALSE */ )
{
    HRESULT hr = S_OK;
    ...
    if( pd3d10DeviceFromApp == NULL )
    {
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            else // if( FAILED(hr) )
            {
				...
				D3D10_DRIVER_TYPE driverType = D3D10_DRIVER_TYPE_HARDWARE;
					...
					//hr = DXUT_Dynamic_D3D10CreateDevice( pAdapter, D3D10_DRIVER_TYPE_HARDWARE, (HMODULE)wrp, pNewDeviceSettings->d3d10.CreateFlags, NULL, D3D10_SDK_VERSION, &pd3d10Device );
                hr = DXUT_Dynamic_D3D10CreateDevice( selectedAdapter, driverType, ( HMODULE )wrp, pNewDeviceSettings->d3d10.CreateFlags, NULL, D3D10_SDK_VERSION, &pd3d10Device );
				//V_RETURN(D3D10CreateDevice(selectedAdapter, driverType, NULL, 0, D3D10_SDK_VERSION, &pDevice));
		...
		if( !GetDXUTState().GetD3D10SwapChain() )
		{
...
void WINAPI DXUTSetD3D10SwapChain( IDXGISwapChain* pSwapChain )
{
	GetDXUTState().SetD3D10SwapChain( pSwapChain );
}