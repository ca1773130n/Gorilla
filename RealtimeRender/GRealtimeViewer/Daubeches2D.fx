...
// DWT
Texture2D<float4>	g_txCoeffs;
Texture2D<uint2>	g_txIndices;
Texture2D<float4>	g_txScales;		
uint			g_iNumCoeffs;
uint			g_iLevel;
uint			g_iSquareIndex;
...
PS_OUT UnpackCoeffsPS(TEX_VS_OUT input) {
	PS_OUT output = (PS_OUT)0;

	output.Color = float4( 0, 0, 0, 1 );
	uint3 msCoord = uint3( uint2(input.Position.xy), 0 );
	if( g_iSquareIndex == 1 || g_iSquareIndex == 3 )
		msCoord.y += 2048 / pow(2, g_iLevel);
	if( g_iSquareIndex == 2 || g_iSquareIndex == 3 )
		msCoord.x += 2048 / pow(2, g_iLevel);

	uint2 index;
	int found = 0;
	for( uint h=0; h < g_iNumCoeffs; h++ )
	{
		for( uint w=0; w < 1024; w++ )
		{
			index = g_txIndices.Load( uint3(w, h, 0), 0 ).xy;
			if( uint(index.x) == msCoord.x && uint(index.y) == msCoord.y )
			{
				output.Color.rgb = g_txCoeffs.Load( uint3(w, h, 0), 0 ).xyz;
				found = 1;
				break;
			}
		}
...
{
	pass P0 {
		SetPixelShader( CompileShader( ps_4_0, UnpackCoeffsPS() ) );
